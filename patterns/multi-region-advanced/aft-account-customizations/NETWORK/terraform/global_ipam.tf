...
module "ipam" {
  ...
  pool_configurations = {
    ...
    "${data.aws_region.secondary.name}" = {
      name                     = "${data.aws_region.secondary.name}-ipam-pool"
      description              = "${data.aws_region.secondary.name}-ipam-pool"
      cidr                     = var.aws_ip_address_plan.secondary_region.cidr_blocks
      locale                   = data.aws_region.secondary.name
      allocation_resource_tags = {}
      tags                     = local.tags
      sub_pools = {
        shared = {
          name                              = "shared-ipam-pool"
          description                       = "shared-ipam-pool" #the description must be the name of the pool
          cidr                              = var.aws_ip_address_plan.secondary_region.shared.cidr_blocks
          locale                            = data.aws_region.secondary.name
          auto_import                       = "true"
          allocation_default_netmask_length = "23"
          allocation_max_netmask_length     = "24"
        ...
        prod = {
          name                              = "prod-ipam-pool"
          description                       = "prod-ipam-pool" #the description must be the name of the pool
          cidr                              = var.aws_ip_address_plan.secondary_region.prod.cidr_blocks
          locale                            = data.aws_region.secondary.name
          auto_import                       = "true"
          allocation_default_netmask_length = "23"
          allocation_max_netmask_length     = "24"
        ...
        stage = {
          name                              = "stage-ipam-pool"
          description                       = "stage-ipam-pool" #the description must be the name of the pool
          cidr                              = var.aws_ip_address_plan.secondary_region.stage.cidr_blocks
          locale                            = data.aws_region.secondary.name
          auto_import                       = "true"
          allocation_default_netmask_length = "23"
          allocation_max_netmask_length     = "24"
        ...
        dev = {
          name                              = "dev-ipam-pool"
          description                       = "dev-ipam-pool" #the description must be the name of the pool
          cidr                              = var.aws_ip_address_plan.secondary_region.dev.cidr_blocks
          locale                            = data.aws_region.primary.name
          auto_import                       = "true"
          allocation_default_netmask_length = "23"
          allocation_max_netmask_length     = "24"
