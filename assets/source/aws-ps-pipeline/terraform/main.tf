...
resource "aws_ssoadmin_managed_policy_attachment" "managed_policy" {
  ...
  instance_arn       = local.sso_instance_arn
  permission_set_arn = aws_ssoadmin_permission_set.permission_set[each.value.ps].arn
...
resource "aws_ssoadmin_permissions_boundary_attachment" "boundary" {
  for_each = { for k, v in local.ps_boundaries : k => v if v.policy != "" || v.customer_policy != "" }

  instance_arn       = local.sso_instance_arn
  permission_set_arn = aws_ssoadmin_permission_set.permission_set[each.value.ps].arn
  permissions_boundary {
    managed_policy_arn = each.value.policy == "" ? null : each.value.policy
...
resource "aws_ssoadmin_account_assignment" "assignment" {
  for_each = { for assignment in local.assignments : "${assignment.ps}_${assignment.principal_id}_${assignment.principal_type}_${assignment.account_id}" => assignment }
  depends_on = [
